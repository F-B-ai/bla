...
> v1.4 aggiunge `mindmovement.assessed`: il Protocollo di Valutazione Neuro-Recettoriale Integrata Mind Movement™ (metodo proprietario del founder) — batteria in `src/data/mindMovementProtocol.ts`, scoring dei 4 domini in `src/domain/mindMovement.ts`, payload = punteggi per dominio + flags. Somministrato dal coach; wording funzionale/benessere, mai diagnosi.
