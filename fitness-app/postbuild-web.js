...
// Registrazione del service worker + AVVISO di versione nuova.
// Il sorgente leggibile sta in web/index.html: se si tocca uno,
// si tocca anche l'altro. Un test lo verifica.
const swScript = `\n    <script>(function () {var SEGNO='essere-riparato';var ricaricato=false;function vuota(){var r=document.getElementById('root');return !r||r.childElementCount===0;}function ricarica(){if(ricaricato)return;ricaricato=true;window.location.reload();}function riparaUnaVolta(){try{if(sessionStorage.getItem(SEGNO))return;sessionStorage.setItem(SEGNO,'1');}catch(e){return;}var lavori=[];if(window.caches&&caches.keys){lavori.push(caches.keys().then(function(nomi){return Promise.all(nomi.map(function(n){return caches.delete(n);}));}));}if(navigator.serviceWorker&&navigator.serviceWorker.getRegistrations){lavori.push(navigator.serviceWorker.getRegistrations().then(function(rs){return Promise.all(rs.map(function(r){return r.unregister();}));}));}Promise.all(lavori).then(ricarica).catch(ricarica);setTimeout(ricarica,4000);}function mostraAvviso(reg) {if (document.getElementById('essere-aggiornamento')) return;var b = document.createElement('div');b.id = 'essere-aggiornamento';b.setAttribute('role', 'status');b.style.cssText = ['position:fixed', 'left:16px', 'right:16px','bottom:calc(env(safe-area-inset-bottom, 0px) + 84px)','z-index:2147483647', 'display:flex', 'align-items:center', 'gap:10px','padding:12px 16px', 'border-radius:14px','background:#1c1c1e', 'border:1px solid #E63946','box-shadow:0 8px 24px rgba(0,0,0,.45)','font:600 14px/1.35 -apple-system,BlinkMacSystemFont,Segoe UI,Roboto,sans-serif','color:#fff', 'cursor:pointer'].join(';');b.innerHTML ='<span style="font-size:18px;line-height:1">↻</span>' +'<span style="flex:1">C’è una versione nuova dell’App.' +'<br><span style="font-weight:400;opacity:.75">Tocca qui per aggiornare.</span></span>';b.addEventListener('click', function () {b.style.opacity = '.5';var atteso = reg && reg.waiting;if (!atteso) { ricarica(); return; }navigator.serviceWorker.addEventListener('controllerchange', ricarica);atteso.postMessage({ type: 'SKIP_WAITING' });setTimeout(ricarica, 3000);});document.body.appendChild(b);}window.addEventListener('load', function () {setTimeout(function(){if(vuota())riparaUnaVolta();else{try{sessionStorage.removeItem(SEGNO);}catch(e){}}},15000);if (!('serviceWorker' in navigator)) return;navigator.serviceWorker.register('/sw.js', { updateViaCache: 'none' }).then(function (reg) {if (reg.waiting && navigator.serviceWorker.controller) mostraAvviso(reg);reg.addEventListener('updatefound', function () {var nuovo = reg.installing;if (!nuovo) return;var eAggiornamento = !!navigator.serviceWorker.controller;nuovo.addEventListener('statechange', function () {if (nuovo.state === 'installed' && eAggiornamento) mostraAvviso(reg);});});var controlla = function () { try { reg.update(); } catch (e) {} };setInterval(controlla, 60000);document.addEventListener('visibilitychange', function () {if (document.visibilityState === 'visible') controlla();});window.addEventListener('focus', controlla);}).catch(function () { /* senza service worker l'App funziona lo stesso */ });});})();</script>`;
// La guardia cerca un SEGNO che non cambia — l'id della fascia — e non
// la firma della `register(...)`: quella è cambiata il 13 settembre 2026
// (è arrivato `updateViaCache`) e una guardia che cercava il testo
// esatto avrebbe smesso di riconoscere lo script già presente,
// iniettandone una seconda copia.
if (!html.includes('essere-aggiornamento')) {
